...
interface Post {
    id: string;
    author_id: string;
    author_type: 'agent' | 'human' | 'student';
    author_name: string;
    content: string;
...
export default function Plaza() {
    ...
    const copyPost = useMutation({
        mutationFn: (postId: string) => postJson(`/api/plaza/posts/${postId}/copy`, {}),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['plaza-posts'] });
            ...
            <div style={{
                ...
                <div>
                    <h1 style={{
                        ...
                        {isChinese ? '学习广场' : 'Study Plaza'}
                    </h1>
                    <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-tertiary)', margin: 0 }}>
                        {isChinese ? '匿名分享解题笔记，跨班级共同学习' : 'Share notes anonymously and learn together'}
                    </p>
                </div>
            </div>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            ...
                            <textarea
                                ...
                                placeholder={isChinese ? "分享你的解题思路、学习笔记或思维导图..." : "Share your problem-solving notes, mind maps..."}
                                maxLength={500}
                                rows={2}
                                style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                                    <input type="checkbox" checked={isAnonymous} onChange={e => setIsAnonymous(e.target.checked)} />
                                    {isChinese ? '匿名发布 (跨班级共享)' : 'Post anonymously (Cross-school sharing)'}
                                </label>
                            </div>
                            <button
                            ...
                            >
                                {createPost.isPending ? t('common.loading', 'Loading...') : (isChinese ? '发布笔记' : 'Share Note')}
                            </button>
                        </div>
                    </div>
                            ...
                            {posts.map((post, idx) => (
                                ...
                                >
                                    ...
                                    <div style={{
                                        ...
                                        />
                                        <ActionBtn
                                            icon={<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5.5 2h5a2 2 0 012 2v8a2 2 0 01-2 2h-5a2 2 0 01-2-2V4a2 2 0 012-2z"></path><path d="M2.5 4.5v9a2 2 0 002 2h6"></path></svg>}
                                            label={post.copies_count || 0}
                                            onClick={() => {
                                                copyPost.mutate(post.id);
                                                alert(isChinese ? '已复制到我的笔记' : 'Copied to my notes');
                                            }}
                                        />
                                    </div>
