        ...
        <div className="company-setup-page">
            ...
            <div className="company-setup-container">
                <div className="company-setup-header">
                    ...
                    <h1>{t('companySetup.title', 'Set up your organization')}</h1>
                    <p className="company-setup-subtitle">
                        {t('companySetup.subtitle', 'Join an existing organization or create a new one to get started.')}
                    </p>
                </div>
                ...
                <div className={`company-setup-panels ${!allowCreate ? 'single' : ''}`}>
                    ...
                    <form className="company-setup-panel" onSubmit={handleJoin}>
                        <div className="company-setup-panel-header">
                            ...
                            <h3>{t('companySetup.joinTitle', 'Join an organization')}</h3>
                        </div>
                        <p className="company-setup-panel-desc">
                            {t('companySetup.joinDesc', 'Enter the join code provided by your teacher or admin.')}
                        </p>
                        <div className="login-field">
                            <label>{t('companySetup.inviteCode', 'Join code')}</label>
                            <input
                                value={inviteCode}
                        ...
                        <button className="login-submit" type="submit" disabled={loading || !inviteCode}>
                            {loading ? <span className="login-spinner" /> : t('companySetup.joinBtn', 'Join')}
                        </button>
                    </form>
                        ...
                        <>
                            ...
                            <form className="company-setup-panel" onSubmit={handleCreate}>
                                <div className="company-setup-panel-header">
                                    ...
                                    <h3>{t('companySetup.createTitle', 'Create an organization')}</h3>
                                </div>
                                <p className="company-setup-panel-desc">
                                    {t('companySetup.createDesc', 'Start a new learning space. You can invite members later.')}
                                </p>
                                <div className="login-field">
                                    <label>{t('companySetup.companyName', 'Organization name')}</label>
                                    <input
                                        value={companyName}
                                        onChange={(e) => setCompanyName(e.target.value)}
                                        required
                                        placeholder={t('companySetup.companyNamePlaceholder', 'e.g. Computer Science Dept.')}
                                    />
                                </div>
                                <button className="login-submit" type="submit" disabled={loading || !companyName}>
                                    {loading ? <span className="login-spinner" /> : t('companySetup.createBtn', 'Create')}
                                </button>
                            </form>
                        </>
                    )}
                ...
                {!allowCreate && (
                    <p className="company-setup-hint">
                        {t('companySetup.contactAdmin', 'Ask your teacher or admin for a join code.')}
                    </p>
                )}
            </div>
        </div>
