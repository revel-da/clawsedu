                            ...
                            <div key={r.id} style={{ borderRadius: '8px', border: '1px solid rgba(16,185,129,0.3)', background: 'rgba(16,185,129,0.05)', overflow: 'hidden' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px' }}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <div style={{ fontSize: '11px', color: 'var(--text-tertiary)' }}>{r.target_agent?.role_description || t('common.assistant', 'Assistant')}</div>
                    ...
                    <div style={{ border: '1px solid rgba(16,185,129,0.5)', borderRadius: '8px', padding: '12px', background: 'var(--bg-elevated)' }}>
                        <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                            <select className="input" value={selectedAgentId} onChange={e => setSelectedAgentId(e.target.value)} style={{ flex: 1, fontSize: '12px' }}>
                                ...
                                {availableAgents.map((a: any) => <option key={a.id} value={a.id}>{a.name} — {a.role_description || t('common.assistant', 'Assistant')}</option>)}
                            </select>
            ...
            <div style={{ padding: '40px', maxWidth: '760px' }}>
                <div className="card" style={{ padding: '20px' }}>
                    <div style={{ fontSize: '16px', fontWeight: 600, marginBottom: '8px' }}>
                        {i18n.language?.startsWith('zh') ? '加载学习助手失败' : 'Failed to load assistant'}
                    </div>
                    <div style={{ fontSize: '13px', color: 'var(--text-tertiary)', marginBottom: '16px' }}>
                        {(error as any)?.message || (i18n.language?.startsWith('zh') ? '学习助手不存在或后端不可用' : 'Assistant not found or backend unavailable')}
                    </div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <button className="btn btn-secondary" onClick={() => refetchAgent()}>{i18n.language?.startsWith('zh') ? '重试' : 'Retry'}</button>
                        <button className="btn btn-ghost" onClick={() => navigate('/agents')}>{i18n.language?.startsWith('zh') ? '返回学习助手列表' : 'Back to assistants'}</button>
                    </div>
                </div>
            </div>
        );
            ...
            <div>
                ...
                <div className="tabs">
                    {TABS.filter(tab => {
                        ...
                        // External agents: only show status, chat, activityLog, settings
                    ...
                    return (
                        <div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '24px' }}>
                                ...
                                {/* External-agent-specific metrics */}
                                {(agent as any)?.agent_type === 'openclaw' && (
                                    <div className="card">
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
                                <div className="card">
                                    <h3 style={{ fontSize: '14px', fontWeight: 600, marginBottom: '12px' }}>
                                        📋 {i18n.language?.startsWith('zh') ? '学习助手信息' : 'Assistant Profile'}
                                    </h3>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                            <span style={{ color: 'var(--text-tertiary)' }}>{i18n.language?.startsWith('zh') ? '创建时间' : 'Created'}</span>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                            <span style={{ color: 'var(--text-tertiary)' }}>{i18n.language?.startsWith('zh') ? '最近活跃' : 'Last Active'}</span>
                                            <span>{agent.last_active_at ? formatDate(agent.last_active_at) : '—'}</span>
                                        </div>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                            <span style={{ color: 'var(--text-tertiary)' }}>🌐 {i18n.language?.startsWith('zh') ? '时区' : 'Timezone'}</span>
                                ...
                                {(agent as any)?.agent_type !== 'openclaw' ? (
                                <div className="card">
                                    <h3 style={{ fontSize: '14px', fontWeight: 600, marginBottom: '12px' }}>{i18n.language?.startsWith('zh') ? '模型配置' : 'Model Config'}</h3>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                            <span style={{ color: 'var(--text-tertiary)' }}>{i18n.language?.startsWith('zh') ? '提供商' : 'Provider'}</span>
                                            <span style={{ textTransform: 'capitalize' }}>{modelProvider}</span>
                                        </div>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                            <span style={{ color: 'var(--text-tertiary)' }}>{i18n.language?.startsWith('zh') ? '上下文轮次' : 'Context Rounds'}</span>
                                ...
                                <div className="card">
                                    <h3 style={{ fontSize: '14px', fontWeight: 600, marginBottom: '12px' }}>
                                        {i18n.language?.startsWith('zh') ? '外部学习助手连接' : 'External Assistant Connection'}
                                    </h3>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                                            <span style={{ color: 'var(--text-tertiary)' }}>{i18n.language?.startsWith('zh') ? '模型' : 'Model'}</span>
                                            <span style={{ color: 'var(--text-secondary)' }}>{i18n.language?.startsWith('zh') ? '由外部学习助手实例管理' : 'Managed by external assistant instance'}</span>
                                        </div>
                                    </div>
                                </div>
                                )}
                            ...
                            <div>
                                ...
                                {(() => {
                                    const scopeLabels: Record<string, string> = {
                                        company: '🏢 ' + t('agent.settings.perm.company', 'Organization-wide'),
                                    ...
                                    return (
                                        <div className="card" style={{ marginBottom: '12px' }}>
                                            ...
                                            <p style={{ fontSize: '12px', color: 'var(--text-tertiary)', marginBottom: '16px' }}>
                                                {t('agent.settings.perm.description', 'Control who can see and interact with this assistant. Only the creator or admin can change this.')}
                                            </p>
                                ...
                                <div className="card" style={{ marginBottom: '12px' }}>
                                    ...
                                    <div style={{
                                        ...
                                        >
                                            <option value="">{t('agent.settings.timezone.default', '↩ Organization default')}</option>
                                ...
                                <div className="card" style={{ marginBottom: '12px' }}>
                                    ...
                                    <p style={{ fontSize: '12px', color: 'var(--text-tertiary)', marginBottom: '16px' }}>
                                        {t('agent.settings.heartbeat.description', 'Periodic awareness check — assistant proactively monitors the plaza and work environment.')}
                                    </p>
