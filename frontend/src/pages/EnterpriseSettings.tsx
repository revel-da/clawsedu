...
interface LLMModel {
    ...
    base_url?: string; api_key_masked?: string; max_tokens_per_day?: number; enabled: boolean; supports_vision?: boolean; max_output_tokens?: number; input_cost_per_million?: number; output_cost_per_million?: number; free_tokens_per_month?: number; created_at: string;
}
...
function estimateDemoCost(inputCost: number, outputCost: number): string {
    const inputTokens = 1000;
    const outputTokens = 1000;
    const totalCents = Math.round((inputTokens / 1000000) * inputCost + (outputTokens / 1000000) * outputCost);
    return toCurrency(totalCents);
}
...
export default function EnterpriseSettings() {
    ...
    const [modelForm, setModelForm] = useState({
        ...
        input_cost_per_million: '' as string,
        output_cost_per_million: '' as string,
        free_tokens_per_month: '' as string,
    });
    ...
    const { data: openmaicServerProviders } = useQuery({
        queryKey: ['openmaic-server-providers'],
        queryFn: () => fetchJson<any>('/enterprise/server-providers'),
        enabled: activeTab === 'llm',
    });
            ...
            <div>
                        ...
                        {openmaicServerProviders && (
                            <div className="card" style={{ marginBottom: '16px' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: '8px', fontSize: '12px' }}>
                                    ...
                                    <div>ASR: {Object.keys(openmaicServerProviders.asr || {}).length}</div>
                                    <div>PDF: {Object.keys(openmaicServerProviders.pdf || {}).length}</div>
                                    <div>Image: {Object.keys(openmaicServerProviders.image || {}).length}</div>
                                    <div>Video: {Object.keys(openmaicServerProviders.video || {}).length}</div>
                                    <div>WebSearch: {Object.keys(openmaicServerProviders.webSearch || {}).length}</div>
                                </div>
                            </div>
                        )}
                        ...
                        {showAddModel && (
                            <div className="card" style={{ marginBottom: '16px' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                    ...
                                    <div className="form-group">
                                        <label className="form-label">Input Cost / 1M Tokens (cents)</label>
                                        <input className="form-input" type="number" min={0} placeholder="例如 50 表示 $0.50" value={modelForm.input_cost_per_million} onChange={e => setModelForm({ ...modelForm, input_cost_per_million: e.target.value })} />
                                    </div>
                                    <div className="form-group">
                                        <label className="form-label">Output Cost / 1M Tokens (cents)</label>
                                        <input className="form-input" type="number" min={0} placeholder="例如 150 表示 $1.50" value={modelForm.output_cost_per_million} onChange={e => setModelForm({ ...modelForm, output_cost_per_million: e.target.value })} />
                                    </div>
                                    <div className="form-group">
                                        <label className="form-label">Free Tokens / Month</label>
                                        <input className="form-input" type="number" min={0} placeholder="例如 1000000" value={modelForm.free_tokens_per_month} onChange={e => setModelForm({ ...modelForm, free_tokens_per_month: e.target.value })} />
                                    </div>
                                </div>
                                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', alignItems: 'center' }}>
                                    ...
                                    <button className="btn btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '6px' }} disabled={!modelForm.model || (!editingModelId && !modelForm.api_key)} onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            if (modelForm.api_key) testData.apiKey = modelForm.api_key;
                                            if (editingModelId) testData.model_id = editingModelId;
                                            const res = await fetch('/api/enterprise/verify-model', {
                                                method: 'POST',
                                                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                                    ...
                                    <button className="btn btn-primary" onClick={() => {
                                        const data = {
                                            ...
                                            input_cost_per_million: modelForm.input_cost_per_million ? Number(modelForm.input_cost_per_million) : null,
                                            output_cost_per_million: modelForm.output_cost_per_million ? Number(modelForm.output_cost_per_million) : null,
                                            free_tokens_per_month: modelForm.free_tokens_per_month ? Number(modelForm.free_tokens_per_month) : null,
                                        };
                                        if (editingModelId) {
                                ...
                                <div key={m.id} className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                        ...
                                        {(m.input_cost_per_million != null || m.output_cost_per_million != null) && (
                                            <span className="badge" style={{ background: 'rgba(34,197,94,0.12)', color: 'rgb(22,163,74)', fontSize: '10px' }}>
                                                费用 {toCurrency(m.input_cost_per_million || 0)} / {toCurrency(m.output_cost_per_million || 0)} 每百万
                                            </span>
                                        )}
                                        <button className="btn btn-ghost" onClick={() => {
                                            ...
                                            setModelForm({
                                                ...
                                                input_cost_per_million: m.input_cost_per_million != null ? String(m.input_cost_per_million) : '',
                                                output_cost_per_million: m.output_cost_per_million != null ? String(m.output_cost_per_million) : '',
                                                free_tokens_per_month: m.free_tokens_per_month != null ? String(m.free_tokens_per_month) : '',
                                            });
                                            setShowAddModel(true);
                            ...
                            <div className="card" style={{ marginTop: '16px' }}>
                                ...
                                {models.filter(m => m.enabled).map((m) => (
                                    <div key={`pricing-${m.id}`} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', gap: '8px', padding: '8px 0', borderTop: '1px solid var(--border-subtle)' }}>
                                        ...
                                        <div style={{ fontSize: '13px' }}>{toCurrency(m.input_cost_per_million || 0)} / {toCurrency(m.output_cost_per_million || 0)}</div>
                                        <div style={{ fontSize: '13px' }}>{estimateDemoCost(m.input_cost_per_million || 0, m.output_cost_per_million || 0)}</div>
                                        <div style={{ fontSize: '13px' }}>{m.free_tokens_per_month ? m.free_tokens_per_month.toLocaleString() : '-'}</div>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                {activeTab === 'info' && (
                    <div>
                        ...
                        {/* ── 0. Company Name ── */}
                        <h3 style={{ marginBottom: '8px' }}>{t('enterprise.companyName.title', 'Organization name')}</h3>
                        ...
                        {/* ── 1. Company Intro ── */}
                        <h3 style={{ marginBottom: '8px' }}>{t('enterprise.companyIntro.title', 'Organization intro')}</h3>
                        <p style={{ fontSize: '12px', color: 'var(--text-tertiary)', marginBottom: '12px' }}>
                            {t('enterprise.companyIntro.description', 'Describe your organization and learning culture. This information is included in every assistant conversation as context.')}
                        </p>
                        <div className="card" style={{ padding: '16px', marginBottom: '24px' }}>
                            <textarea
                                className="form-input"
                                ...
                                placeholder={
                                    isChinese
                                        ? `# 机构名称\nClawsEdu\n\n# 简介\n校园 AI 学习伙伴平台\n学习 · 练习 · 项目 · 复盘\n\n面向学生、老师与管理者。\n强调更清晰、更专注、更好上手。`
                                        : `# Organization Name\nClawsEdu\n\n# About\nCampus Learning Assistant Platform\nStudy · Practice · Projects · Review\n\nBuilt for students, teachers, and admins.\nClear, focused, and easy to start.`
                                }
                                style={{
                            ...
                            <div style={{ marginTop: '12px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                                ...
                                <span style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginLeft: 'auto' }}>
                                    💡 {t('enterprise.companyIntro.hint', 'This content appears in every assistant\'s system prompt')}
                                </span>
                            </div>
                        </div>
                        ...
                        <div style={{ marginTop: '32px', padding: '16px', border: '1px solid var(--status-error, #e53e3e)', borderRadius: '8px' }}>
                            ...
                            <p style={{ fontSize: '12px', color: 'var(--text-tertiary)', marginBottom: '12px' }}>
                                {t('enterprise.deleteCompanyDesc', 'Permanently delete this organization and all its data including assistants, models, tools, and skills. This action cannot be undone.')}
                            </p>
                            <button
                                className="btn"
                                onClick={async () => {
                                    const name = document.querySelector<HTMLInputElement>('.company-name-input')?.value || selectedTenantId;
                                    if (!confirm(t('enterprise.deleteCompanyConfirm', 'Are you sure you want to delete this organization and ALL its data? This cannot be undone.'))) return;
                                    try {
                            ...
                            >
                                {t('enterprise.deleteCompany', 'Delete This Organization')}
                            </button>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'quotas' && (
                    <div>
                        <h3 style={{ marginBottom: '4px' }}>{isChinese ? '默认用户配额' : 'Default User Quotas'}</h3>
                        <p style={{ fontSize: '12px', color: 'var(--text-tertiary)', marginBottom: '16px' }}>
                            {isChinese ? '仅对新注册用户生效；不影响已有用户。' : 'These defaults apply to newly registered users. Existing users are not affected.'}
                        </p>
                        <div className="card" style={{ padding: '16px' }}>
                            {/* ── Conversation Limits ── */}
                            <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '10px' }}>{isChinese ? '对话配额' : 'Conversation Limits'}</div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                                <div className="form-group">
                                    <label className="form-label">{isChinese ? '消息额度' : 'Message Limit'}</label>
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: '4px' }}>{isChinese ? '每个周期的最大消息数' : 'Max messages per period'}</div>
                                </div>
                                <div className="form-group">
                                    <label className="form-label">{isChinese ? '重置周期' : 'Message Period'}</label>
                                    <select className="form-input" value={quotaForm.default_message_period}
                                        ...
                                        <option value="permanent">{isChinese ? '永久' : 'Permanent'}</option>
                            ...
                            {/* ── Agent Limits ── */}
                            <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '10px' }}>{isChinese ? '学习助手配额' : 'Assistant Limits'}</div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                                <div className="form-group">
                                    <label className="form-label">{isChinese ? '最多学习助手' : 'Max assistants'}</label>
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: '4px' }}>{isChinese ? '每个用户可创建的学习助手数量' : 'Assistants a user can create'}</div>
                                </div>
                                <div className="form-group">
                                    <label className="form-label">{isChinese ? '学习助手存活时长（小时）' : 'Assistant TTL (hours)'}</label>
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: '4px' }}>{isChinese ? '从创建起自动过期' : 'Auto-expiry time from creation'}</div>
                                </div>
                                <div className="form-group">
                                    <label className="form-label">{isChinese ? '每日 LLM 调用 / 学习助手' : 'Daily LLM Calls / Assistant'}</label>
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: '4px' }}>{isChinese ? '每个学习助手每天最多调用次数' : 'Max calls per assistant per day'}</div>
                                </div>
                            </div>

                            {/* ── System Limits ── */}
                            <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '10px' }}>{isChinese ? '系统限制' : 'System'}</div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                                <div className="form-group">
                                    <label className="form-label">{isChinese ? '最小心跳间隔（分钟）' : 'Min Heartbeat Interval (min)'}</label>
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: '4px' }}>{isChinese ? '对所有学习助手生效的最小心跳间隔' : 'Minimum heartbeat interval for all assistants'}</div>
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                                ...
                                <div className="form-group">
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: '4px' }}>
                                        {t('enterprise.quotas.minPollIntervalDesc', 'Organization-wide floor: assistants cannot poll faster than this')}
                                    </div>
                                </div>
                                <div className="form-group">
                                    ...
                                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: '4px' }}>
                                        {t('enterprise.quotas.maxWebhookRateDesc', 'Organization-wide ceiling: max webhook hits per minute per assistant')}
                                    </div>
                                </div>
                            </div>
