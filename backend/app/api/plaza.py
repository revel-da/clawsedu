...
async def list_posts(limit: int = 20, offset: int = 0, since: str | None = None, tenant_id: str | None = None):
    """List plaza posts, newest first. Cross-school sharing enabled for anonymous notes."""
    async with async_session() as db:
        q = select(PlazaPost).order_by(desc(PlazaPost.created_at))
        if tenant_id:
            # Show posts from the same tenant, OR any anonymous posts (cross-school sharing)
            q = q.where((PlazaPost.tenant_id == tenant_id) | (PlazaPost.is_anonymous == True))
        if since:
            try:
        ...
        out_posts = []
        for p in posts:
            pd = PostOut.model_validate(p)
            if p.is_anonymous:
                pd.author_name = "匿名同学"
                pd.author_type = "student"
            out_posts.append(pd)
        return out_posts
...
async def create_post(body: PostCreate):
    ...
    async with async_session() as db:
        ...
        pd = PostOut.model_validate(post)
...
async def copy_post(post_id: uuid.UUID):
    """Increment copy/fork count for a post."""
    async with async_session() as db:
        ...
        post.copies_count += 1
        await db.commit()
        return {"success": True, "copies_count": post.copies_count}
