...
def _load_env_section(env_map: dict[str, str], include_models: bool) -> dict:
    result: dict[str, dict] = {}
    for prefix, provider_id in env_map.items():
        api_key = os.getenv(f"{prefix}_API_KEY")
        base_url = os.getenv(f"{prefix}_BASE_URL")
        models_raw = os.getenv(f"{prefix}_MODELS")
        models = [m.strip() for m in (models_raw or "").split(",") if m.strip()]
        if not api_key and not base_url and not models:
            continue
        item = result.get(provider_id, {})
        if base_url:
...
def _llm_server_providers() -> dict:
    result: dict[str, dict] = {}
    for spec in get_provider_manifest():
        ...
        if default_base_url:
            item["baseUrl"] = default_base_url
        result[provider] = item
    env_overrides = _load_env_section(LLM_ENV_MAP, include_models=True)
    for provider, item in env_overrides.items():
        merged = result.get(provider, {})
        merged.update(item)
...
class ClassroomVerifyRequest(BaseModel):
    apiKey: str | None = None
    baseUrl: str | None = None
    model: str
    providerType: str | None = None
    requiresApiKey: bool | None = True
    model_id: str | None = None


async def _probe_llm(provider: str, model: str, api_key: str, base_url: str | None):
...
async def classroom_server_providers(
    current_user: User = Depends(get_current_user),
):
    return {
        "providers": _llm_server_providers(),
        "tts": _load_env_section(TTS_ENV_MAP, include_models=False),
        "asr": _load_env_section(ASR_ENV_MAP, include_models=False),
        "pdf": _load_env_section(PDF_ENV_MAP, include_models=False),
        "image": _load_env_section(IMAGE_ENV_MAP, include_models=False),
        "video": _load_env_section(VIDEO_ENV_MAP, include_models=False),
        "webSearch": _load_env_section(WEB_SEARCH_ENV_MAP, include_models=False),
    }
...
async def test_llm_model(
    ...
    try:
        latency_ms, reply = await _probe_llm(data.provider, data.model, api_key, data.base_url)
        return {"success": True, "latency_ms": latency_ms, "reply": reply}
    except Exception as e:
        return {"success": False, "latency_ms": 0, "error": str(e)[:500]}


@router.post("/verify-model")
async def classroom_verify_model(
    data: ClassroomVerifyRequest,
    current_user: User = Depends(get_current_admin),
    db: AsyncSession = Depends(get_db),
):
    provider = data.providerType or "custom"
    api_key = data.apiKey
    if not api_key and data.model_id:
        result = await db.execute(select(LLMModel).where(LLMModel.id == data.model_id))
        existing = result.scalar_one_or_none()
        if existing:
            api_key = existing.api_key_encrypted
    if (data.requiresApiKey is None or data.requiresApiKey) and not api_key:
        return {"success": False, "message": "API key is required", "response": "", "latency_ms": 0}
    try:
        latency_ms, reply = await _probe_llm(provider, data.model, api_key or "no-key", data.baseUrl)
        return {
            "success": True,
        ...
        if "401" in msg or "Unauthorized" in msg:
            ...
            msg = "Model not found or API endpoint error"
        elif "429" in msg:
            msg = "API rate limit exceeded, please try again later"
        elif "ENOTFOUND" in msg or "ECONNREFUSED" in msg:
            msg = "Cannot connect to API server, please check the Base URL"
...
async def add_llm_model(
    ...
    model = LLMModel(
        ...
        free_tokens_per_month=data.free_tokens_per_month,
...
async def update_llm_model(
    ...
    try:
        ...
        if data.input_cost_per_million is not None:
            model.input_cost_per_million = data.input_cost_per_million
        if data.output_cost_per_million is not None:
            model.output_cost_per_million = data.output_cost_per_million
        if data.free_tokens_per_month is not None:
            model.free_tokens_per_month = data.free_tokens_per_month
