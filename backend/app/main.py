...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            try:
                await conn.execute(_text("ALTER TYPE channel_type_enum ADD VALUE IF NOT EXISTS 'atlassian'"))
            except Exception:
                pass
            for stmt in [
                "ALTER TABLE llm_models ADD COLUMN input_cost_per_million INTEGER",
                "ALTER TABLE llm_models ADD COLUMN output_cost_per_million INTEGER",
                "ALTER TABLE llm_models ADD COLUMN free_tokens_per_month INTEGER",
                "ALTER TABLE plaza_posts ADD COLUMN is_anonymous BOOLEAN DEFAULT FALSE",
                "ALTER TABLE plaza_posts ADD COLUMN copies_count INTEGER DEFAULT 0",
            ]:
                try:
